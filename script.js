	...
	*/
	...
	function septemberWeekday(day) {
		return [
			...
			"Freitag",
			"Samstag"
		][new Date(Date.UTC(2026, 8, day)).getUTCDay()];
	}
	var testQuestions = [
		{
			question: "Wie laut darf im QuietSpace gesprochen werden?",
			answers: [
				"In normaler Lautstärke",
				"Maximal im Flüsterton",
				"Gar nicht"
			],
			correct: 1
		},
		{
			question: "Was gilt für Getränke im QuietSpace?",
			answers: [
				"Nur geschlossene Flaschen",
				"Alle Getränke sind erlaubt",
				"Keine offenen Getränke"
			],
			correct: 2
		},
		{
			question: "Was sollst du tun, wenn du eine Reservation nicht brauchst?",
			answers: [
				"Nichts",
	...
	function seatClass(state, selected) {
		if (selected) return "border-[#0a615e] bg-[#dff0ed] text-[#075b58] ring-4 ring-[#dff0ed]/70 shadow-sm";
		if (state === "free") return "border-[#a9d9cc] bg-white text-[#245b55] hover:border-[#21a179] hover:shadow-md";
		return "border-[#dce5e4] bg-[#e8eeee] text-[#809594]";
	}
	function Home() {
		const [mode, setMode] = (0, import_react.useState)("learner-ready");
		const [view, setView] = (0, import_react.useState)("overview");
		const [selectedSeat, setSelectedSeat] = (0, import_react.useState)("B5");
		const [selectedDay, setSelectedDay] = (0, import_react.useState)(7);
		const [fromTime, setFromTime] = (0, import_react.useState)("13:00");
		const [toTime, setToTime] = (0, import_react.useState)("17:00");
		const [recurring, setRecurring] = (0, import_react.useState)(false);
		const [dialogOpen, setDialogOpen] = (0, import_react.useState)(false);
		const [notice, setNotice] = (0, import_react.useState)(null);
		const [testStep, setTestStep] = (0, import_react.useState)(0);
		const [testAnswers, setTestAnswers] = (0, import_react.useState)([
			null,
			null,
			null
		]);
		const [revokedSeat, setRevokedSeat] = (0, import_react.useState)(null);
		const [removedSeatIds, setRemovedSeatIds] = (0, import_react.useState)([]);
		const isTeacher = mode === "teacher";
		const testPassed = mode !== "learner-pending";
		const selectedWeekday = septemberWeekday(selectedDay);
		...
		const effectiveSeats = (0, import_react.useMemo)(() => seats.map((seat) => removedSeatIds.includes(seat.id) ? {
			id: seat.id,
			state: "free"
		} : seat), [removedSeatIds]);
		(0, import_react.useEffect)(() => {
			...
			Promise.resolve(context.registerTool({
				name: "create_quietspace_reservation",
				title: "QuietSpace-Platz reservieren",
				description: "Reserviert einen freien QuietSpace-Arbeitsplatz und zeigt die Reservation im Kalender an.",
				inputSchema: {
					type: "object",
					properties: {
						seat: {
							type: "string",
							enum: seats.filter((seat) => seat.state === "free").map((seat) => seat.id)
						},
						day: {
							type: "integer",
							minimum: 1,
				...
				execute(input) {
					if (mode === "learner-pending") throw new Error("Der Regeltest muss zuerst bestanden werden.");
					const value = input;
					const seat = seats.find((item) => item.id === value.seat && item.state === "free");
					if (!seat || !value.day || !value.from || !value.to) throw new Error("Ungültige Reservationsdaten.");
					setSelectedSeat(seat.id);
					setSelectedDay(value.day);
					setFromTime(value.from);
					setToTime(value.to);
					setNotice(`Platz ${seat.id} wurde für den ${value.day}. September reserviert.`);
					setView("calendar");
					return {
						status: "confirmed",
						seat: seat.id,
						date: `2026-09-${String(value.day).padStart(2, "0")}`,
						from: value.from,
						to: value.to
					};
				}
			}, { signal: lifecycle.signal })).catch(() => void 0);
			return () => lifecycle.abort();
		}, [mode]);
		function switchMode(next) {
			setMode(next);
			setNotice(null);
			setView(next === "learner-pending" ? "overview" : "overview");
			setTestStep(0);
			setTestAnswers([
				null,
				null,
		...
		function navigate(next) {
			if (!testPassed && (next === "reserve" || next === "calendar")) {
				setView("test");
				return;
			}
			setView(next);
		...
		function chooseDay(day, muted) {
			if (muted || isTeacher) return;
			setSelectedDay(day);
			setView("reserve");
		}
		function confirmReservation() {
			setDialogOpen(false);
			setNotice(`Platz ${selectedSeat} ist am ${selectedDay}. September von ${fromTime} bis ${toTime} reserviert.`);
			setView("calendar");
		}
		function completeTest() {
			if (!testQuestions.every((question, index) => testAnswers[index] === question.correct)) {
				setNotice("Noch nicht ganz. Prüfe deine Antworten und versuche es nochmals.");
				setTestStep(0);
				return;
			}
			setMode("learner-ready");
			setNotice("Regeltest bestanden. Reservationen sind jetzt freigeschaltet.");
			setView("overview");
		}
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
			className: "min-h-screen bg-[#f3f7f7] text-[#10282a]",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("header", {
					className: "sticky top-0 z-30 border-b border-[#d6e3e1] bg-white/95 backdrop-blur-xl",
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "mx-auto flex min-h-[76px] max-w-[1500px] flex-nowrap items-center gap-5 px-4 py-3 sm:px-6 lg:px-8",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
								onClick: () => navigate("overview"),
								className: "flex shrink-0 items-center gap-3 text-left",
								"aria-label": "QuietSpace Übersicht öffnen",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "grid size-10 place-items-center rounded-[14px] bg-[#0a615e] text-white shadow-[0_8px_20px_rgba(10,97,94,.22)]",
									...
									className: "block text-[19px] tracking-[-.03em]",
									children: "QuietSpace"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "block text-xs font-medium text-[#668081]",
									children: "BBBaden"
								})] })]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("nav", {
								...
								children: [navigation$1.filter(({ id }) => testPassed || id !== "reserve" && id !== "calendar").map(({ id, label, icon: Icon }) => {
									return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
										onClick: () => navigate(id),
										"aria-current": view === id ? "page" : void 0,
										className: `flex min-w-fit items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${view === id ? "bg-[#dff0ed] text-[#075b58]" : "text-[#506667] hover:bg-[#f0f5f4] hover:text-[#183334]"}`,
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Icon, { className: "size-[17px]" }), label]
									}, id);
								}), !testPassed && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
									onClick: () => setView("test"),
									className: `flex min-w-fit items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${view === "test" ? "bg-[#fff1d6] text-[#81540a]" : "text-[#7b6846] hover:bg-[#fff8eb]"}`,
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(GraduationCap, { className: "size-[17px]" }), "Regeltest"]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "flex shrink-0 items-center gap-2 sm:gap-3",
								children: [
									!isTeacher && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
										onClick: () => setView(testPassed ? "overview" : "test"),
										className: `hidden items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold sm:flex ${testPassed ? "border-[#cfe0de] bg-[#eef7f5] text-[#0a615e]" : "border-[#f0d6a6] bg-[#fff7e8] text-[#8b5c0a]"}`,
										children: [testPassed ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "size-4" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LockKeyhole, { className: "size-4" }), testPassed ? "Regeltest bestanden" : "Regeltest offen"]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "hidden items-center gap-2 rounded-xl border border-[#d5e1e0] bg-[#f8fbfa] p-1 md:flex",
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "pl-2 text-[11px] font-bold uppercase tracking-[.08em] text-[#75898a]",
											children: "Prototype"
										}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
											value: mode,
											onChange: (event) => switchMode(event.target.value),
											"aria-label": "Prototyp-Ansicht wählen",
											className: "h-8 min-w-44 rounded-lg border-0 bg-white px-2 text-sm font-semibold shadow-sm outline-none focus:ring-2 focus:ring-[#21a179]",
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
													value: "learner-pending",
													children: "Paul · Test offen"
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
													value: "learner-ready",
													children: "Paul · freigeschaltet"
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
													value: "teacher",
													children: "Frank · Lehrperson"
												})
											]
										})]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "flex items-center gap-2 rounded-full p-1.5 pr-2",
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "grid size-9 place-items-center rounded-full bg-[#dff0ed] text-sm font-bold text-[#0a615e]",
											...
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
												...
												className: "text-[11px] text-[#6e8283]",
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
							...
							onChange: (event) => switchMode(event.target.value),
							"aria-label": "Prototyp-Ansicht wählen",
							className: "h-10 w-full rounded-lg border border-[#d5e1e0] bg-white px-2 text-sm font-semibold outline-none focus:ring-2 focus:ring-[#21a179]",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "learner-pending",
									children: "Paul · Test offen"
						...
						className: "min-w-0 px-4 py-7 sm:px-6 lg:px-8 lg:py-10",
						children: [
							notice && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								role: "status",
								className: `mb-6 flex items-start gap-3 rounded-2xl border px-4 py-3.5 text-sm font-medium ${notice.startsWith("Noch") ? "border-[#efd39c] bg-[#fff8eb] text-[#744b09]" : "border-[#b9ddd4] bg-[#eaf7f3] text-[#075b58]"}`,
								children: [
									notice.startsWith("Noch") ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TriangleAlert, { className: "mt-0.5 size-5 shrink-0" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "mt-0.5 size-5 shrink-0" }),
							...
							view === "overview" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Overview, {
								...
								onRevoke: setRevokedSeat
							}),
							view === "reserve" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ReserveView, {
								isTeacher,
								selectedSeat,
								setSelectedSeat,
								selectedDay,
							...
							view === "calendar" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CalendarView, {
								isTeacher,
								selectedSeat,
								selectedDay,
								onDay: chooseDay,
							...
							view === "rules" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RulesView, {}),
							view === "test" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TestView, {
								step: testStep,
								setStep: setTestStep,
								answers: testAnswers,
				...
				dialogOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "fixed inset-0 z-50 grid place-items-center bg-[#0c2526]/25 p-4 backdrop-blur-sm",
					role: "presentation",
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
								...
								children: "Reservation bestätigen"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
								className: "mt-2 text-sm text-[#677b7c]",
								children: "Prüfe die Angaben, bevor du den Arbeitsplatz reservierst."
							})] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("dl", {
								className: "grid grid-cols-2 gap-3 rounded-2xl bg-[#f2f7f6] p-4 text-sm",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("dt", {
										className: "text-xs text-[#718687]",
										children: "Platz"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("dd", {
										className: "mt-1 font-bold",
										children: selectedSeat
									})] }),
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("dt", {
										className: "text-xs text-[#718687]",
										children: "Datum"
										...
										className: "mt-1 font-bold",
										children: [selectedDay, ". September 2026"]
									})] }),
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("dt", {
										className: "text-xs text-[#718687]",
										children: "Zeit"
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("dt", {
										className: "text-xs text-[#718687]",
										children: "Wiederholung"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("dd", {
										className: "mt-1 font-bold",
										children: recurring ? `Jeden ${selectedWeekday}` : "Einmalig"
									})] })
								]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
									...
									className: "bg-[#0a615e] hover:bg-[#084f4d]",
									onClick: confirmReservation,
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Check, { className: "size-4" }), " Verbindlich reservieren"]
								})]
							})
						]
					})
				}),
				revokedSeat && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "fixed inset-0 z-50 grid place-items-center bg-[#0c2526]/25 p-4 backdrop-blur-sm",
					role: "presentation",
					onMouseDown: () => setRevokedSeat(null),
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
						role: "alertdialog",
						"aria-modal": "true",
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "grid size-10 place-items-center rounded-xl bg-[#fff0ef] text-[#b8423d]",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Trash2, { className: "size-5" })
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
								id: "revoke-title",
								className: "mt-4 text-lg font-bold",
								children: "Reservation aufheben?"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
								className: "mt-2 text-sm leading-6 text-[#677b7c]",
								children: [
									"Die Reservation von ",
									revokedSeat.person,
									" für Platz ",
									revokedSeat.id,
									" wird gelöscht. Die betroffene Person erhält eine Information."
								]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
									variant: "outline",
									onClick: () => setRevokedSeat(null),
									children: "Abbrechen"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
									variant: "destructive",
									onClick: () => {
										setRemovedSeatIds((current) => [...current, revokedSeat.id]);
										setNotice(`Die Reservation für Platz ${revokedSeat.id} wurde aufgehoben.`);
										setRevokedSeat(null);
									},
									children: "Reservation aufheben"
								})]
	...
	function PageHeading({ eyebrow, title, text, action }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "mb-2 text-sm font-semibold text-[#0a716c]",
					children: eyebrow
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					className: "text-[clamp(2rem,4vw,3.25rem)] font-bold leading-[1.04] tracking-[-.045em]",
					children: title
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "mt-3 max-w-2xl text-base leading-7 text-[#5d7273]",
					children: text
				})
			] }), action]
		});
	}
	function Overview({ isTeacher, testPassed, onNavigate, seats, onRevoke }) {
		if (!testPassed) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
			eyebrow: "Willkommen im QuietSpace",
			title: "Zuerst die Regeln. Dann dein Platz.",
			text: "Bevor du reservieren kannst, bestätigst du in einem kurzen Test, dass du die wichtigsten QuietSpace-Regeln kennst."
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
			className: "relative overflow-hidden rounded-[28px] bg-[#0c3637] p-7 text-white shadow-[0_20px_55px_rgba(12,54,55,.2)] sm:p-10",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute -right-24 -top-24 size-72 rounded-full bg-[#2bb895]/15 blur-2xl" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "relative max-w-2xl",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold text-[#a8e6d9]",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(LockKeyhole, { className: "size-4" }), " Reservationen gesperrt"]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
						className: "mt-7 text-3xl font-bold tracking-[-.04em] sm:text-4xl",
						children: "Regeltest absolvieren"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "mt-3 max-w-xl text-base leading-7 text-white/68",
						children: "Drei kurze Fragen. Du kannst die Regeln jederzeit unter «Infos & Regeln» nachlesen."
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "mt-8 flex flex-wrap gap-3",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
							className: "h-11 bg-[#7fd7c5] px-5 text-[#0c3637] hover:bg-[#97e5d5]",
							onClick: () => onNavigate("test"),
							children: ["Test starten ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowRight, { className: "size-4" })]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
							variant: "outline",
							className: "h-11 border-white/20 bg-white/5 px-5 text-white hover:bg-white/10",
							onClick: () => onNavigate("rules"),
							children: "Regeln ansehen"
						})]
					})
				]
			})]
		...
		if (isTeacher) {
			const occupied = seats.filter((seat) => seat.state === "occupied");
			return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
				eyebrow: "Montag, 7. September 2026",
				title: "Belegung im Blick.",
				text: "Sieh sofort, wer im QuietSpace arbeitet, und verwalte bestehende Reservationen.",
				action: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
					className: "h-11 bg-[#0a615e] px-5 hover:bg-[#084f4d]",
					onClick: () => onNavigate("calendar"),
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CalendarDays, { className: "size-4" }), " Kalender öffnen"]
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "grid gap-5 xl:grid-cols-[1.15fr_.85fr]",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RoomMap, {
					seats,
					selectedSeat: "",
					teacher: true,
					onSelect: (seat) => seat.state === "occupied" && onRevoke(seat)
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "space-y-5",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(StatCard, {
						icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Users, {}),
						label: "Aktuell belegt",
						value: `${occupied.length} von 14`,
						detail: "50 % Auslastung"
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
						className: "rounded-[24px] border border-[#d4e1df] bg-white p-5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-xs font-bold uppercase tracking-[.12em] text-[#789091]",
							children: "Anwesende Personen"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "mt-4 space-y-2",
							children: occupied.slice(0, 4).map((seat) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
								onClick: () => onRevoke(seat),
								className: "flex w-full items-center gap-3 rounded-xl p-2 text-left hover:bg-[#f2f7f6]",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
										className: "grid size-9 place-items-center rounded-full bg-[#e8efee] text-xs font-bold text-[#526968]",
										children: seat.initials
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
										className: "block text-sm",
										children: seat.person
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
										className: "text-xs text-[#728586]",
										children: [
											"Platz ",
											seat.id,
											" · bis 17:00"
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRight, { className: "ml-auto size-4 text-[#92a3a2]" })
								]
							}, seat.id))
						})]
					})]
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
			eyebrow: "Montag, 7. September 2026",
			title: "Guten Morgen, Paul.",
			text: "Finde einen ruhigen Arbeitsplatz oder prüfe deine nächsten Reservationen.",
			action: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
				className: "h-11 bg-[#0a615e] px-5 hover:bg-[#084f4d]",
				onClick: () => onNavigate("reserve"),
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Armchair, { className: "size-4" }), " Platz reservieren"]
			})
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "grid gap-5 xl:grid-cols-[1.12fr_.88fr]",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RoomMap, {
				seats,
				selectedSeat: "B5",
				onSelect: () => onNavigate("reserve")
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "grid gap-5 sm:grid-cols-2 xl:grid-cols-1",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
					className: "rounded-[24px] bg-[#0c3637] p-6 text-white shadow-[0_18px_38px_rgba(12,54,55,.16)]",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex items-center justify-between",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white/80",
								children: "Nächste Reservation"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CalendarDays, { className: "size-5 text-[#7fd7c5]" })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-8 text-4xl font-bold tracking-[-.05em]",
							children: "B5"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-2 text-sm text-white/65",
							children: "Heute, 13:00–17:00"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "mt-6 flex items-center gap-4 border-t border-white/10 pt-5 text-xs text-white/75",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "flex items-center gap-2",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(MapPin, { className: "size-4" }), " Raum 2.14"]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "flex items-center gap-2",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Clock3, { className: "size-4" }), " 4 Std."]
							})]
						})
					]
					...
					label: "Auslastung",
					value: "7 von 14 frei",
					detail: "Am Nachmittag sind mehrere Einzelplätze verfügbar."
				})]
			})]
		})] });
	}
	function StatCard({ icon, label, value, detail }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
			className: "rounded-[24px] border border-[#d4e1df] bg-white p-6",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center justify-between",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						...
						className: "grid size-11 place-items-center rounded-2xl bg-[#e9f4f2] text-[#0a615e] [&>svg]:size-5",
						children: icon
					})]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "mt-6 h-2.5 overflow-hidden rounded-full bg-[#e7efee]",
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "h-full w-1/2 rounded-full bg-[#21a179]" })
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "mt-3 text-sm leading-6 text-[#647879]",
					children: detail
				})
			]
		});
	}
	function RoomMap({ seats, selectedSeat, onSelect, teacher = false }) {
		...
		const renderSeat = (id, vertical = false) => {
			const seat = byId.get(id);
			return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				onClick: () => onSelect(seat),
				disabled: !teacher && seat.state === "occupied",
				"aria-label": teacher && seat.person ? `${seat.id}, reserviert von ${seat.person}` : `${seat.id}, ${seat.state === "free" ? "frei" : "belegt"}`,
				className: `relative grid min-h-[62px] flex-1 place-items-center px-2 text-sm font-bold transition ${vertical ? "min-w-[86px]" : ""} ${seatClass(seat.state, selectedSeat === seat.id)} ${teacher && seat.state === "occupied" ? "cursor-pointer hover:bg-[#fff1f0]" : ""}`,
				children: [teacher && seat.person ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
				...
				}) : seat.id, /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: `absolute right-2 top-2 size-2.5 rounded-full border-2 border-white ${seat.state === "occupied" ? "bg-[#a5b5b4]" : "bg-[#21a179]"}` })]
			}, id);
		};
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
			className: "overflow-hidden rounded-[24px] border border-[#d4e1df] bg-white shadow-[0_14px_40px_rgba(19,55,57,.06)]",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex flex-wrap items-center justify-between gap-3 border-b border-[#e2ebea] px-5 py-5 sm:px-6",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-xs font-bold uppercase tracking-[.12em] text-[#789091]",
					children: "Heute · 13:00–17:00"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
					className: "mt-1 text-xl font-bold tracking-[-.03em]",
					children: "Tischplan"
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex gap-4 text-xs font-semibold text-[#607677]",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "flex items-center gap-1.5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "size-2.5 rounded-full bg-[#21a179]" }), " Frei"]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "flex items-center gap-1.5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "size-2.5 rounded-full bg-[#a5b5b4]" }), " Belegt"]
					})]
				})]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "min-h-[430px] overflow-x-auto bg-[#f7faf9] p-5 sm:p-7",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "mb-6 flex items-center justify-between",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "text-xs font-semibold uppercase tracking-[.12em] text-[#8aa09f]",
							children: "Fensterseite"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "rounded-lg border border-[#d7e3e2] bg-white px-3 py-1.5 text-xs font-semibold text-[#607677]",
							children: "QuietSpace 2.14"
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "grid min-w-[620px] grid-cols-[1fr_1fr_112px] gap-8",
						children: [tableGroups.map((group) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
							"aria-label": `Tischgruppe ${group.label}`,
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h3", {
								className: "mb-3 text-center text-sm font-bold text-[#526b6c]",
								children: ["Tischgruppe ", group.label]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "space-y-7",
								children: group.pairs.map(([left, right]) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "overflow-hidden rounded-2xl border-2 border-[#c8d7d5] bg-[#edf3f2] p-1 shadow-[0_7px_18px_rgba(22,62,63,.06)]",
									children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "flex gap-1 overflow-hidden rounded-xl",
										children: [renderSeat(left), renderSeat(right)]
									})
								}, left))
							})]
						}, group.label)), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
							"aria-label": "Tischgruppe C",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", {
								className: "mb-3 text-center text-sm font-bold text-[#526b6c]",
								children: "Tischgruppe C"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "mx-auto mt-10 w-[100px] overflow-hidden rounded-2xl border-2 border-[#c8d7d5] bg-[#edf3f2] p-1 shadow-[0_7px_18px_rgba(22,62,63,.06)]",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "flex flex-col gap-1 overflow-hidden rounded-xl",
									children: [renderSeat("C1", true), renderSeat("C2", true)]
								})
							})]
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "mt-7 flex items-center justify-between border-t border-dashed border-[#cbd9d7] pt-4 text-xs font-semibold uppercase tracking-[.1em] text-[#93a5a4]",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Eingang" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Ruhezone" })]
					})
				]
			})]
		});
	}
	function ReserveView({ isTeacher, selectedSeat, setSelectedSeat, selectedDay, setSelectedDay, fromTime, setFromTime, toTime, setToTime, recurring, setRecurring, seats, onConfirm }) {
		const selectedWeekday = septemberWeekday(selectedDay);
		const weeks = [
			[
				{
		...
		const [weekIndex, setWeekIndex] = (0, import_react.useState)(() => selectedDay >= 21 ? 2 : selectedDay >= 14 ? 1 : 0);
		const dayChoices = weeks[weekIndex];
		const changeWeek = (next) => {
			const bounded = Math.max(0, Math.min(weeks.length - 1, next));
			setWeekIndex(bounded);
			setSelectedDay(weeks[bounded][0].day);
		};
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
			eyebrow: "Reservation erstellen",
			title: "Dein Platz für konzentrierte Arbeit.",
			text: "Wähle Datum, Zeit und einen freien Arbeitsplatz. Belegte Plätze sind klar markiert."
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "grid gap-5 xl:grid-cols-[330px_1fr]",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", {
				className: "space-y-5 rounded-[24px] border border-[#d4e1df] bg-white p-5 sm:p-6",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", {
							className: "text-xs font-bold uppercase tracking-[.1em] text-[#6f8586]",
							children: "Datum"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
										...
										className: "text-[11px] font-medium text-[#778b8c]",
										children: ["Woche ", weekIndex + 2]
									})]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
									size: "icon",
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							...
							children: dayChoices.map(({ day, weekday }) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
								onClick: () => setSelectedDay(day),
								className: `rounded-xl px-1 py-2.5 text-center transition ${selectedDay === day ? "bg-[#0a615e] text-white shadow-md" : "bg-[#f0f5f4] text-[#476061] hover:bg-[#e2edeb]"}`,
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "block text-[10px] font-bold uppercase opacity-70",
									children: weekday
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "border-t border-[#e3ebea] pt-5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-xs font-bold uppercase tracking-[.1em] text-[#6f8586]",
							children: "Zeitfenster"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "mt-3 grid grid-cols-2 gap-3",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "text-xs text-[#667b7c]",
								children: ["Von", /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
									value: fromTime,
									onChange: (event) => setFromTime(event.target.value),
									className: "mt-1.5 h-10 w-full rounded-lg border border-[#d5e1e0] bg-white px-2 text-sm font-semibold outline-none focus:ring-2 focus:ring-[#21a179]",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "08:00" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "09:00" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "12:00" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "13:00" })
									]
								})]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "text-xs text-[#667b7c]",
								children: ["Bis", /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
									value: toTime,
									...
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "12:00" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "13:00" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { children: "17:00" })
									]
								})]
							})]
						})]
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
						className: "flex cursor-pointer items-start gap-3 rounded-xl bg-[#f3f7f6] p-3.5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "checkbox",
							checked: recurring,
							onChange: (event) => setRecurring(event.target.checked),
							className: "mt-0.5 size-4 accent-[#0a615e]"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
							className: "block text-sm",
							children: "Wöchentlich wiederholen"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
							className: "mt-1 block text-xs leading-5 text-[#718586]",
							children: [
								"Jeden ",
								selectedWeekday,
								" von ",
								...
								" bis ",
								toTime,
								", bis Ende des Semesters. Bei hoher Auslastung kann die Serie angepasst werden."
							]
						})] })]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "rounded-xl border border-[#cfe1de] bg-[#ebf6f3] p-3.5",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
								className: "text-xs text-[#647d7a]",
								children: "Deine Auswahl"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
								className: "mt-1 text-lg font-bold text-[#075b58]",
								children: ["Platz ", selectedSeat]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
								className: "mt-0.5 text-xs text-[#56706e]",
								children: [
									selectedDay,
									".09.2026 · ",
									fromTime,
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
						className: "h-11 w-full bg-[#0a615e] hover:bg-[#084f4d]",
						onClick: onConfirm,
						children: ["Reservation prüfen ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowRight, { className: "size-4" })]
					})
				]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RoomMap, {
				seats,
				selectedSeat,
				teacher: isTeacher,
				onSelect: (seat) => !isTeacher && seat.state === "free" && setSelectedSeat(seat.id)
			})]
		})] });
	}
	function CalendarView({ isTeacher, selectedSeat, selectedDay, onDay, seats, onRevoke }) {
		const [monthIndex, setMonthIndex] = (0, import_react.useState)(1);
		const visibleMonth = calendarMonths[monthIndex];
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
			eyebrow: isTeacher ? "Reservationsübersicht" : "Deine Reservationen",
			title: visibleMonth.label,
			text: isTeacher ? "Wähle einen belegten Platz, um die Reservation und die zuständige Person zu sehen." : "Der ganze Monat ist auf einen Blick sichtbar. Wähle einen freien Tag für eine neue Reservation.",
			action: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						...
						onClick: () => setMonthIndex((month) => Math.max(0, month - 1)),
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronLeft, {})
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "min-w-24 text-center text-xs font-bold text-[#637879]",
						children: [
							monthIndex + 1,
							" / ",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						...
						onClick: () => setMonthIndex((month) => Math.min(calendarMonths.length - 1, month + 1)),
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRight, {})
					})
				]
			})
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "grid gap-5 xl:grid-cols-[320px_1fr]",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", {
				className: "rounded-[24px] border border-[#d4e1df] bg-white p-5",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-xs font-bold uppercase tracking-[.11em] text-[#789091]",
					children: isTeacher ? "Belegung dieses Monats" : "Meine Reservationen"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "mt-4 space-y-3",
					children: isTeacher ? seats.filter((seat) => seat.state === "occupied").slice(0, 5).map((seat) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						onClick: () => onRevoke(seat),
						className: "flex w-full items-center gap-3 rounded-xl border border-[#e0e9e8] p-3 text-left hover:border-[#e3b4b0] hover:bg-[#fff8f7]",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "grid size-9 place-items-center rounded-full bg-[#e9efee] text-xs font-bold",
								children: seat.initials
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
								className: "block text-sm",
								...
								className: "text-xs text-[#728586]",
								children: [seat.id, " · 08:00–17:00"]
							})] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Trash2, { className: "ml-auto size-4 text-[#b8423d]" })
						]
					}, seat.id)) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ReservationCard, {
							seat: "B5",
							date: "7. September",
							time: "13:00–17:00"
						}),
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
							...
							onClick: () => onDay(21),
							children: "Weitere Reservation"
						})
					] })
				})]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
				className: "overflow-hidden rounded-[24px] border border-[#d4e1df] bg-white",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "grid grid-cols-5 border-b border-[#e1e9e8] bg-[#f6f9f8]",
					children: [
						"Mo",
						"Di",
						"Mi",
						...
						className: "px-2 py-3 text-center text-xs font-bold uppercase tracking-[.1em] text-[#75898a]",
						children: day
					}, day))
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "grid grid-cols-5",
					children: visibleMonth.days.map((item, index) => {
						const reserved = monthIndex === 1 && !isTeacher && !item.muted && booked.has(item.day);
						return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							onClick: () => onDay(item.day, item.muted),
							disabled: item.muted || isTeacher || monthIndex !== 1,
							className: `relative min-h-24 border-r border-t border-[#e5eceb] p-2.5 text-left transition sm:min-h-28 sm:p-3 ${item.muted ? "bg-[#fafcfc] text-[#b1bebd]" : reserved ? "bg-[#edf7f4]" : monthIndex === 1 && !isTeacher ? "bg-white hover:bg-[#f3f8f7]" : "bg-white"}`,
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: `grid size-9 place-items-center rounded-full text-sm font-bold ${reserved ? "bg-[#0a615e] text-white" : ""}`,
									children: item.day
								}),
								reserved && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "mt-3 block rounded-lg bg-white px-2 py-1.5 text-[11px] font-semibold text-[#0a615e] shadow-sm",
									children: item.day === 7 ? "B5 · 13–17" : `${selectedSeat} · 08–17`
								}),
								isTeacher && monthIndex === 1 && !item.muted && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "mt-3 block rounded-lg bg-[#102f31] px-2 py-1.5 text-[11px] font-semibold text-white",
									children: item.day === 7 ? "7 belegt" : "Verfügbar"
								})
							]
						}, `${item.day}-${monthIndex}-${index}`);
					})
				})]
			})]
		})] });
	}
	function ReservationCard({ seat, date, time }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "rounded-2xl bg-[#0c3637] p-4 text-white",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center justify-between",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
	...
	function RulesView() {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
			eyebrow: "Infos & Regeln",
			title: "Ein Raum für konzentriertes Arbeiten.",
			text: "Der QuietSpace öffnet nach der Sommerpause am 7. September 2026. Damit alle ungestört arbeiten können, gelten wenige klare Regeln."
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "grid gap-5 lg:grid-cols-[1fr_.8fr]",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
				className: "rounded-[26px] bg-[#0c3637] p-7 text-white sm:p-9",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-[#a8e6d9]",
						children: "QuietSpace 2.14"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
						className: "mt-6 max-w-xl text-3xl font-bold leading-tight tracking-[-.04em]",
						children: "Ruhe ist eine gemeinsame Entscheidung."
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "mt-4 max-w-xl text-base leading-7 text-white/68",
						children: "Die Plätze stehen Lernenden und Lehrpersonen während der Schulöffnungszeiten zur Verfügung."
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "mt-8 grid gap-3 sm:grid-cols-2",
						children: [
							{
								icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Volume2, {}),
								title: "Leise arbeiten",
								text: "Gespräche sind maximal im Flüsterton erlaubt."
							},
							{
								icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Info, {}),
								title: "Arbeitsplatz freigeben",
								text: "Lösche Reservationen, wenn du den Platz nicht benötigst."
							},
							{
								icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShieldCheck, {}),
								title: "Sauber hinterlassen",
								text: "Räume deinen Arbeitsplatz nach der Nutzung vollständig auf."
							},
							{
								icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Clock3, {}),
								title: "Pünktlich erscheinen",
								text: "Nicht genutzte Plätze können nach 15 Minuten freigegeben werden."
							}
						].map((rule) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "rounded-2xl border border-white/10 bg-white/[.06] p-4",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "grid size-9 place-items-center rounded-xl bg-[#7fd7c5]/15 text-[#7fd7c5] [&>svg]:size-4",
									children: rule.icon
								}),
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						...
						className: "mt-4 space-y-3 text-sm leading-6 text-[#52696a]",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
								className: "flex gap-3",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "mt-1 size-4 shrink-0 text-[#21a179]" }), "Keine offenen Getränke"]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
								className: "flex gap-3",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "mt-1 size-4 shrink-0 text-[#21a179]" }), "Reservationen nur bei tatsächlicher Nutzung"]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
								className: "flex gap-3",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "mt-1 size-4 shrink-0 text-[#21a179]" }), "Langzeitreservationen können bei hoher Auslastung eingeschränkt werden"]
							})
						]
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
					className: "rounded-[24px] border border-[#eed9ad] bg-[#fff9ee] p-6",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TriangleAlert, { className: "size-5 text-[#a96d0a]" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", {
							className: "mt-3 text-base font-bold",
							children: "Reservation geändert?"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-2 text-sm leading-6 text-[#735f39]",
							children: "Wenn eine Serie eingeschränkt oder eine Reservation gelöscht wird, informiert dich QuietSpace in deiner Übersicht."
						})
					]
				})]
			})]
	...
	function TestView({ step, setStep, answers, setAnswers, onComplete }) {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageHeading, {
					eyebrow: "Regeltest",
					title: "Drei Fragen bis zur Freischaltung.",
					text: "Lies jede Frage sorgfältig. Du kannst deine Antwort vor dem Abschluss jederzeit ändern."
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
					className: "overflow-hidden rounded-[28px] border border-[#d4e1df] bg-white shadow-[0_18px_55px_rgba(19,55,57,.08)]",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "border-b border-[#e2ebea] px-6 py-5",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "flex items-center justify-between text-xs font-bold text-[#6f8384]",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
									"Frage ",
									step + 1,
									" von ",
									testQuestions.length
								] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [Math.round((step + 1) / testQuestions.length * 100), " %"] })]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "mt-3 h-2 overflow-hidden rounded-full bg-[#e7efee]",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "h-full rounded-full bg-[#21a179] transition-all",
									style: { width: `${(step + 1) / testQuestions.length * 100}%` }
								})
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("fieldset", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("legend", {
								className: "text-2xl font-bold leading-tight tracking-[-.035em]",
								children: current.question
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "mt-7 grid gap-3",
								children: current.answers.map((answer, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
									className: `flex cursor-pointer items-center gap-4 rounded-2xl border p-4 transition ${selected === index ? "border-[#0a615e] bg-[#edf7f4] ring-2 ring-[#dff0ed]" : "border-[#dce6e5] hover:bg-[#f7faf9]"}`,
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
										type: "radio",
										name: `question-${step}`,
										value: index,
										...
										className: "size-4 accent-[#0a615e]"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex items-center justify-between border-t border-[#e2ebea] bg-[#f8faf9] px-6 py-4",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
								variant: "ghost",
								disabled: step === 0,
								onClick: () => setStep(step - 1),
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowLeft, {}), " Zurück"]
							}), step < testQuestions.length - 1 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
								className: "bg-[#0a615e] hover:bg-[#084f4d]",
								disabled: selected === null,
								onClick: () => setStep(step + 1),
								children: ["Weiter ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowRight, {})]
							}) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
								className: "bg-[#0a615e] hover:bg-[#084f4d]",
								disabled: selected === null,
								onClick: onComplete,
								children: ["Test abschliessen ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Check, {})]
							})]
						})
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					...
					className: "mx-auto mt-5 flex items-center gap-2 text-xs font-semibold text-[#607677] hover:text-[#0a615e]",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RotateCcw, { className: "size-3.5" }), " Antworten zurücksetzen"]
				})
			]
		});
	}
	//#endregion
	//#region standalone-entry.tsx
	var root = document.getElementById("quietspace-app");
	if (!root) throw new Error("QuietSpace konnte nicht gestartet werden.");
	(0, import_client.createRoot)(root).render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Home, {}));
	//#endregion
})();
